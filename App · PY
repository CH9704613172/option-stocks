"""
╔══════════════════════════════════════════════════════════════════╗
║        NIFTY OPTIONS STOCK SCREENER  v5  —  Streamlit UI        ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
<div class="main-header">
  ...
  <p>RSI({RSI_PERIOD}) &nbsp;│&nbsp; MACD({MACD_FAST},{MACD_SLOW},{MACD_SIGNAL})
     ...
     &nbsp;│&nbsp; ATR({ATR_PERIOD})
     &nbsp;│&nbsp; {datetime.now().strftime("%d %b %Y  %H:%M")} IST</p>
</div>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════
#  CORE LOGIC  (unchanged from v5 CLI)
# ══════════════════════════════════════════════════════════════════
...
def detect_candle_pattern(o, h, l, c) -> str:
    ...
    is_bull    = c >= o
    if body_pct < 0.05:                                   return "Doji"
    if body_pct >= 0.85:                                  return "Bull Marubozu" if is_bull else "Bear Marubozu"
    if lower_wick >= body*2 and upper_wick <= body*0.4:   return "Hammer"        if is_bull else "Hanging Man"
    if upper_wick >= body*2 and lower_wick <= body*0.4:   return "Inv Hammer"    if is_bull else "Shooting Star"
    if upper_wick >= body*1.5 and lower_wick >= body*1.5: return "Spinning Top"
    if body_pct >= 0.6:                                   return "Bull Candle"   if is_bull else "Bear Candle"
    return "Bull Small" if is_bull else "Bear Small"


_PATTERN_SCORE = {
    "Bull Marubozu": (3,0,"Strong bull body"),  "Bear Marubozu": (0,3,"Strong bear body"),
    "Hammer":        (2,0,"Reversal up"),        "Hanging Man":   (0,2,"Reversal down"),
    "Inv Hammer":    (1,0,"Possible bull rev"),  "Shooting Star": (0,2,"Possible bear rev"),
    "Bull Candle":   (2,0,"Trend continuation"), "Bear Candle":   (0,2,"Trend continuation"),
    "Bull Small":    (1,0,"Mild bullish"),        "Bear Small":    (0,1,"Mild bearish"),
    "Spinning Top":  (0,0,"Indecision"),          "Doji":          (0,0,"Indecision"),
}


def next_day_bias(sig: dict) -> dict:
    bias = dict(nd_bias="WAIT", nd_action="—", nd_confidence="—",
                nd_entry=None, nd_target=None, nd_stoploss=None, nd_reason="—")
    close   = sig.get("close");   atr   = sig.get("atr")
    pattern = sig.get("candle_pattern","Doji");  rsi = sig.get("rsi")
    hist    = sig.get("macd_hist");  ma_fast = sig.get("ma_fast");  ma_slow = sig.get("ma_slow")
    vol_r   = sig.get("vol_ratio",0); candle = sig.get("candle","—")
    ...
    if ma_fast > ma_slow:   bull_score += 2
    elif ma_fast < ma_slow: bear_score += 2
    ...
    if bull_score > bear_score + 1:
        direction = "LONG";  action = "BUY CE (Call)"
        ...
        direction = "SHORT"; action = "BUY PE (Put)"
        conf = "HIGH" if bear_score >= 6 else ("MEDIUM" if bear_score >= 4 else "LOW")
        reason = f"{pattern} | RSI {rsi:.1f} | Hist {hist:+.3f} | MA {'above' if ma_fast>ma_slow else 'below'} | Vol {vol_r:.1f}× | {pat_desc}"
    else:
        bias.update(nd_bias="WAIT", nd_action="Conflicting signals", nd_confidence="LOW",
                    nd_reason=f"Bull {bull_score} vs Bear {bear_score} — too close")
        return bias

    if atr and atr > 0:
        if direction == "LONG":
            entry, target, sl = round(close,2), round(close+atr*TARGET_ATR_MULT,2), round(close-atr*SL_ATR_MULT,2)
        else:
            entry, target, sl = round(close,2), round(close-atr*TARGET_ATR_MULT,2), round(close+atr*SL_ATR_MULT,2)
    else:
        pct = close * 0.015
        if direction == "LONG":
            entry, target, sl = close, round(close+pct,2), round(close-pct*0.5,2)
        else:
            entry, target, sl = close, round(close-pct,2), round(close+pct*0.5,2)

    bias.update(nd_bias=direction, nd_action=action, nd_confidence=conf,
                nd_entry=entry, nd_target=target, nd_stoploss=sl, nd_reason=reason)
    return bias
...
def classify_signal(df):
    ...
    empty = dict(strict_signal="NEUTRAL", relaxed_signal="NEUTRAL", cross="—",
    ...
    ma_fast_now  = _f(today["MA_FAST"]);  ma_slow_now  = _f(today["MA_SLOW"])
    ma_fast_prev = _f(prev["MA_FAST"]);   ma_slow_prev = _f(prev["MA_SLOW"])
    rsi          = _f(today["RSI"])
    macd_val     = _f(today[macd_col]);   macd_sig  = _f(today[macds_col])
    macd_hist    = _f(today[macdh_col])
    close_now    = _f(today["Close"]);    open_now  = _f(today["Open"])
    high_now     = _f(today["High"]);     low_now   = _f(today["Low"])
    vol_now      = _f(today["Volume"]);   prev_high = _f(prev["High"])
    prev_low     = _f(prev["Low"]);       prev_vol  = _f(prev["Volume"])
    ...
    if   golden and rsi > RSI_BUY_LEVEL and macd_bull and bull_candle: strict,cross = "BUY","GOLDEN"
    elif death  and rsi < RSI_SEL_LEVEL and macd_bear and bear_candle: strict,cross = "SELL","DEATH"
    elif above  and rsi > RSI_BUY_LEVEL and macd_bull and bull_candle: strict,cross = "BULLISH","ABOVE"
    elif below  and rsi < RSI_SEL_LEVEL and macd_bear and bear_candle: strict,cross = "BEARISH","BELOW"
    else:                                                               strict,cross = "NEUTRAL","—"

    if   golden and rsi > RSI_BUY_LEVEL and macd_bull: relaxed,cross = "BUY","GOLDEN"
    elif death  and rsi < RSI_SEL_LEVEL and macd_bear: relaxed,cross = "SELL","DEATH"
    elif above  and rsi > RSI_BUY_LEVEL and macd_bull: relaxed,cross = "BULLISH","ABOVE"
    elif below  and rsi < RSI_SEL_LEVEL and macd_bear: relaxed,cross = "BEARISH","BELOW"
    else:                                               relaxed       = "NEUTRAL"

    cross_lbl = cross if (strict!="NEUTRAL" or relaxed!="NEUTRAL") else "—"

    sig = dict(strict_signal=strict, relaxed_signal=relaxed, cross=cross_lbl,
               close=round(close_now,2), prev_high=round(prev_high,2), prev_low=round(prev_low,2),
               atr=round(atr_val,2) if atr_val else None,
               vol_today=int(vol_now), vol_ratio=round(vol_ratio,2),
               candle=candle_lbl, candle_pattern=candle_pattern,
               ma_fast=round(ma_fast_now,2), ma_slow=round(ma_slow_now,2),
               rsi=round(rsi,2), macd=round(macd_val,4),
...
# ══════════════════════════════════════════════════════════════════
#  SCREENER RUNNER  (with Streamlit progress)
# ══════════════════════════════════════════════════════════════════
@st.cache_data(ttl=900, show_spinner=False)   # cache 15 min
def run_screener(stocks_tuple):
    ...
    for ticker in stocks:
        ...
        rows.append(sig)
        time.sleep(SLEEP_BETWEEN)
    return pd.DataFrame(rows) if rows else pd.DataFrame(), skipped
...
def _render_signal_table(df, label, emoji):
    ...
    rename = {"ticker":"Stock","close":"Close","prev_high":"Prev High","prev_low":"Prev Low",
               "candle":"Breakout","candle_pattern":"Pattern","vol_ratio":"Vol ×",
               "rsi":"RSI","macd_hist":"MACD Hist","ma_fast":f"MA{MA_FAST}","ma_slow":f"MA{MA_SLOW}"}
...
def _render_nd_table(df):
    ...
    rename   = {"ticker":"Stock","close":"Close","nd_bias":"Next Day","nd_confidence":"Confidence",
                "nd_action":"Action","nd_entry":"Entry","nd_target":"Target",
                "nd_stoploss":"Stop Loss","atr":"ATR","nd_reason":"Reason"}
...
if run_btn or "results" in st.session_state:
    ...
    if run_btn or "results" not in st.session_state:
        ...
        for i, ticker in enumerate(stocks):
            pct  = int((i+1)/len(stocks)*100)
            name = ticker.replace(".NS","")
            progress_bar.progress(pct, text=f"Scanning {name}  [{i+1}/{len(stocks)}]")
            df = fetch_data(ticker)
            if df is None:
                skipped.append(name)
            else:
    ...
    st.markdown(f"<p style='color:#adb5bd; font-size:0.8rem;'>Last scan: {scan_time}  │  "
                f"Scanned: {len(results)} stocks</p>", unsafe_allow_html=True)
    ...
    with tabs[9]:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"nifty_signals_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
            mime="text/csv",
        )
